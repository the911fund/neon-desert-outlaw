...
export class ParticleSystem {
  ...
  private readonly trackContainer?: Container;
  ...
  constructor(container?: Container, options: ParticleSystemOptions = {}) {
    ...
    this.trackContainer = options.trackContainer;
    this.particleContainer = options.particleContainer;
    this.random = options.random ?? Math.random;
    this.enableRendering =
      options.enableRendering ??
      Boolean(container || options.trackContainer || options.particleContainer);
  ...
  private spawnParticle(config: {
    ...
    if (particle.sprite) {
      ...
      const targetContainer = this.getContainerForKind(config.kind);
      if (targetContainer && !particle.sprite.parent) {
        targetContainer.addChild(particle.sprite);
      }
  ...
  private getContainerForKind(kind: ParticleKind): Container | undefined {
    if (kind === 'track') {
      return this.trackContainer ?? this.defaultContainer;
    }
    return this.particleContainer ?? this.defaultContainer;
  }
